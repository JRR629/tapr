...
| 11 | 🟠 | High | `/api/profile` zod validation | ✅ done | schema in `lib/validation/athleteProfile.ts`; strips unknown cols; verified 10/10 vs real payloads + build green. NOTE: onboarding `actions.ts` insert still unvalidated (separate follow-up) |
...
| 21 | 🟢 | Med | Security headers | todo | SECURITY SEC-4 |
